...
> 2026-09-06 發佈順序更新：以 DECISIONS D-28 與 `REMOTE_CI_DELIVERY.md` 為準。下方 §5 完整本機 CI 後直接 main 的順序已被候選遠端驗證流程取代。獨立二審仍必要，走 `scripts/codex_review.sh`，不使用 §3 歷史 MCP／stdin diff 範例；不得更换指定模型。
